...
TOKEN = os.getenv("TELEGRAM_BOT_TOKEN", "7622295622:AAG-Qhn9JUORpf5TBxS0Dij_dQN-x70qJ-8")
...
def help_command(message):
    ...
    - **/bgmi <target> <port> <duration> <threads>** - ⚡ Launch an attack.
...
def handle_sharp(message):
    ...
    if len(command) != 5 or not command[3].isdigit() or not command[4].isdigit():
        bot.reply_to(message, "Invalid format! Use: `/bgmi <target> <port> <duration> <threads>`", parse_mode='Markdown')
        return
